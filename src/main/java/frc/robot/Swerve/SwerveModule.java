...
public class SwerveModule {
    ...
    public SwerveModule(int driveID, int turnID, int encoderID, 
        ...
        driveCustomSpark = new CustomSpark(driveID, driveInverted);
        driveRelativeEncoder = driveCustomSpark.relativeEncoder;
        turnCustomSpark = new CustomSpark(turnID, turnInverted);
        ...
        previousDrivePosition = driveRelativeEncoder.getPosition() * Conversions.Constants.driveMetresPerRotation;
    }

    public void combinedPercentageSet(double driveSet, double turnSet) {
        driveCustomSpark.percentageSet(driveSet);
        turnCustomSpark.percentageSet(turnSet);
    }
    ...
    public double getAbsoluteModuleAngle() {
        double temp = -(180 + absoluteEncoderInvert * (absoluteEncoder.getAbsolutePosition()) * Conversions.Constants.turnEncoderDegreesPerRotation + absoluteEncoderOffset);
    ...
    public double getWheelMetresPosition() {
        return driveRelativeEncoder.getPosition() * Conversions.Constants.driveMetresPerRotation;
    }

    public double getWheelMetresVelocity() {
        return driveRelativeEncoder.getVelocity() * Conversions.Constants.driveMetresPerRotation / 60d;
    }
