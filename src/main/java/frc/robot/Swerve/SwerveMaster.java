...
public class SwerveMaster {
    ...
    public SwerveMaster(NetworkTableInstance inst) {
        ...
        mySwerveModules[0] = new SwerveModule(IDs.Constants.leftUpDrive, IDs.Constants.leftUpTurn, IDs.Constants.leftUpEncoder, 
            Inversions.Constants.leftUpDrive, Inversions.Constants.leftUpTurn, Inversions.Constants.leftUpEncoder, 
            Offsets.Constants.leftUpOffset);
        mySwerveModules[1] = new SwerveModule(IDs.Constants.leftDownDrive, IDs.Constants.leftDownTurn, IDs.Constants.leftDownEncoder, 
            Inversions.Constants.leftDownDrive, Inversions.Constants.leftDownTurn, Inversions.Constants.leftDownEncoder, 
            Offsets.Constants.leftDownOffset);
        mySwerveModules[2] = new SwerveModule(IDs.Constants.rightUpDrive, IDs.Constants.rightUpTurn, IDs.Constants.rightUpEncoder, 
            Inversions.Constants.rightUpDrive, Inversions.Constants.rightUpTurn, Inversions.Constants.rightUpEncoder, 
            Offsets.Constants.rightUpOffset);
        mySwerveModules[3] = new SwerveModule(IDs.Constants.rightDownDrive, IDs.Constants.rightDownTurn, IDs.Constants.rightDownEncoder, 
            Inversions.Constants.rightDownDrive, Inversions.Constants.rightDownTurn, Inversions.Constants.rightDownEncoder, 
            Offsets.Constants.rightDownOffset);

        myPigeon = new Pigeon2(IDs.Constants.pigeon2, "rio");
    ...
    public void combinedLimitedSet(double[] driveSets, double[] turnSets) {
        for(int i = 0; i < 4; i++) {
            mySwerveModules[i].combinedPercentageSet(driveSets[i], turnSets[i]);
        }
    }
    ...
    public void alignModules() {
        mySwerveModules[0].alignPosition(XYH, XYH[2], Measurements.Constants.leftUpModulePos);
        mySwerveModules[1].alignPosition(XYH, XYH[2], Measurements.Constants.leftDownModulePos);
        mySwerveModules[2].alignPosition(XYH, XYH[2], Measurements.Constants.rightUpModulePos);
        mySwerveModules[3].alignPosition(XYH, XYH[2], Measurements.Constants.rightDownModulePos);
    }

    public void teleopUpdate() {
        if(Controllers.Dynamics.driveController.getL1Button()) {
            Robot.swerveTranslationalFactor = Math.max(0.05d, Robot.swerveTranslationalFactor - 0.005d);
        } else if(Controllers.Dynamics.driveController.getL2Button()) {
            Robot.swerveTranslationalFactor = Math.min(1d, Robot.swerveTranslationalFactor + 0.005d);
        }

        if(Controllers.Dynamics.driveController.getR1Button()) {
            Robot.swerveRotationalFactor = Math.max(0.05d, Robot.swerveRotationalFactor - 0.005d);
        } else if(Controllers.Dynamics.driveController.getR2Button()) {
            Robot.swerveRotationalFactor = Math.min(1d, Robot.swerveRotationalFactor + 0.005d);
        }

        if(Controllers.Dynamics.driveController.getShareButtonPressed()) {
            resetHeading(180d);
        }
        ...
        double inputX = Math.abs(Controllers.Dynamics.driveController.getLeftX()) < Controllers.Constants.driveControllerDeadband 
            ? 0d 
            : Controllers.Dynamics.driveController.getLeftX() * Robot.swerveTranslationalFactor;
        double inputY = Math.abs(Controllers.Dynamics.driveController.getLeftY()) < Controllers.Constants.driveControllerDeadband 
            ? 0d 
            : Controllers.Dynamics.driveController.getLeftY() * Robot.swerveTranslationalFactor;
        double inputH = Math.abs(Controllers.Dynamics.driveController.getRightX()) < Controllers.Constants.driveControllerDeadband 
            ? 0d 
            : Controllers.Dynamics.driveController.getRightX() * Robot.swerveRotationalFactor;
        ...
        for(int i = 0; i < 4; i++) {
            ...
            if(Math.abs(desiredTurnSet) > Math.abs(desiredPlusSet)) {
                ...
                mySwerveModules[i].combinedPercentageSet(-desiredMagnitude, desiredPlusSet);
                ...
                mySwerveModules[i].combinedPercentageSet(-desiredMagnitude, desiredMinusSet);
            } else {
                //mySwerveModules[i].combinedRPMSet(desiredMagnitude * Constants.maxRPMs.drive, desiredTurnSet * Constants.maxRPMs.turn);
                mySwerveModules[i].combinedPercentageSet(desiredMagnitude, desiredTurnSet);
            }
        }
    }
}
